const Marriage = require('../models/Marriage');
const Resident = require('../models/Resident');
const mongoose = require('mongoose');
const { parsePagination, sendList } = require('../lib/pagination');

exports.getMarriages = async (req, res) => {
  try {
    const { status, kebele, search } = req.query;
    const { hasPage, page, limit, skip } = parsePagination(req.query, { defaultLimit: 25, maxLimit: 100 });
    const filter = {};
    if (status) filter.status = status;
    if (kebele) filter.kebele = kebele;
    if (search) {
      filter.$or = [
        { husbandName: { $regex: search, $options: 'i' } },
        { wifeName: { $regex: search, $options: 'i' } },
      ];
    }

    const total = await Marriage.countDocuments(filter);
    let query = Marriage.find(filter)
      .populate('husbandId', 'fullName fatherName idNo')
      .populate('wifeId', 'fullName fatherName idNo')
      .sort({ createdAt: -1 });
    if (hasPage) query = query.skip(skip).limit(limit);

    const data = await query;
    return sendList(res, { data, total, page, limit, hasPage });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};

exports.createMarriage = async (req, res) => {
  try {
    const documents = {};
    if (req.files) {
      Object.keys(req.files).forEach(f => { documents[f] = req.files[f][0].filename; });
    }

    // ── Best-effort relational linking ──
    let { husbandId, wifeId } = req.body;
    if (husbandId && mongoose.Types.ObjectId.isValid(husbandId)) {
      const found = await Resident.findById(husbandId);
      husbandId = found ? found._id : null;
    } else {
      husbandId = null;
    }
    if (wifeId && mongoose.Types.ObjectId.isValid(wifeId)) {
      const found = await Resident.findById(wifeId);
      wifeId = found ? found._id : null;
    } else {
      wifeId = null;
    }

    const marriage = await Marriage.create({
      ...req.body,
      husbandId,
      wifeId,
      documents,
      registeredBy: req.user.name,
      registeredById: req.user._id,
      status: 'pending',
    });
    res.status(201).json({ success: true, data: marriage });
  } catch (err) {
    // ── Duplicate key error (code 11000) ──
    if (err.code === 11000) {
      return res.status(400).json({
        message: 'A marriage record with the same husband, wife, and date already exists.',
      });
    }
    res.status(500).json({ message: err.message });
  }
};

exports.updateStatus = async (req, res) => {
  try {
    const { status, note } = req.body;
    if (!['approved', 'rejected'].includes(status)) {
      return res.status(400).json({ message: 'Status must be approved or rejected.' });
    }
    const marriage = await Marriage.findByIdAndUpdate(
      req.params.id,
      { status, note: note || '', reviewedBy: req.user._id, reviewedAt: new Date() },
      { new: true }
    );
    if (!marriage) return res.status(404).json({ message: 'Marriage record not found.' });
    res.json({ success: true, data: marriage });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};

exports.deleteMarriage = async (req, res) => {
  try {
    const marriage = await Marriage.findByIdAndUpdate(
      req.params.id,
      { isDeleted: true, deletedAt: new Date(), deletedBy: req.user._id },
      { new: true }
    );
    if (!marriage) return res.status(404).json({ message: 'Marriage record not found.' });
    res.json({ success: true, message: 'Marriage record deleted.' });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};